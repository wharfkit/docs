...
The one caveat to this is that the `data` field on the `Action` needs to be serialized, which is what the `Action` Antelope data type helps achieve. 

Once serialized, the actual anatomy of an `Action` is represented by:
...
The serialized data contained within the `data` field is encoded using the [Struct](#) defined originating from the [ABI](#).

## Usage

A new smart contract `Action` can be created in multiple ways, depending on whether the data is serialized. 
...
#### Using serialized data from a Struct
